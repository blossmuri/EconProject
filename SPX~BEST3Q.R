...
FPEplot <- ts(rev(FPE$YoY3Q[(1 + 60*(i - 1)) : (60*i)]), start = c((2023-5*i), 4), frequency = 12)

plot(SPXplot, main = "SPX and FPE Series", ylim= c(-45, 60), xlab = "Time", ylab = "% Change") ; lines(FPEplot, col = "blue")

plot(SPX$YoY[(1 + 60*(i - 1)) : (60*i)],FPE$YoY3Q[(1 + 60*(i - 1)) : (60*i)],
     xlab = "SPX", ylab = "FPE3Q", main = paste((2023-5*i),"~", (2023-5*(i-1) )))


# test 1. Cross-Correlation analysis

FPEseries <- as.vector(as.numeric(FPE$YoY3Q[(1 + 60*(i - 1)) : (60*i)]))
...
if (lag_time >= 0) {
  ...
  for (k in 1:60) {
    ...
    FPEsubset <- as.vector(as.numeric(FPE$YoY3Q[( 60*(i - 1) + k) : (60*(i-1) + k + (lag_time))]))
    X[k, ] <-  FPEsubset
