...
plot(SPX$YoY[(1 + 60*(i - 1)) : (60*i)],FPE$YoY2Q[(1 + 60*(i - 1)) : (60*i)],
     xlab = "SPX", ylab = "FPE2Q", main = paste((2023-5*i),"~", (2023-5*(i-1) )))


# test 1. Cross-Correlation analysis

FPEseries <- as.vector(as.numeric(FPE$YoY2Q[(1 + 60*(i - 1)) : (60*i)]))
...
if (lag_time >= 0) {
  ...
  for (k in 1:60) {
    ...
    FPEsubset <- as.vector(as.numeric(FPE$YoY2Q[( 60*(i - 1) + k) : (60*(i-1) + k + (lag_time))]))
    X[k, ] <-  FPEsubset
